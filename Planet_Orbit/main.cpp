...
main(int argc, char *argv[])
{
	...
	{
#ifndef COARSE
	float fx = 0., fy = 0., fz = 0., myPartial = 0.;
	#ifndef STATIC
		...
		for (int i = 0; i < NUMBODIES; i++)
		{
			...
			#ifndef FINE
				#ifndef STATIC
					#pragma omp parallel for default(none) reduction(+:fx,fy,fz), schedule(static), private(myPartial)
				#endif // STATIC
				#ifndef DYNAMIC
					#pragma omp parallel for default(none) reduction(+:fx,fy,fz), schedule(dynamic), private(myPartial)
				#endif // !DYNAMIC
			#endif // !_FINE
	...
	float megaBodiesPerSec = ((float)(NUMBODIES*NUMBODIES*NUMSTEPS) / (time1 - time0) / 1000000.);
	char test[5];
	printf("Number of Threads:%d\n", NUMT);
	printf("Number of Bodies:%d\n", NUMBODIES);
	printf("Number of Steps:%d\n", NUMSTEPS);
	printf("Megas Bodies Per Second: %8.2f\n", megaBodiesPerSec);
	scanf_s("%s",test);
	return 0;
}
